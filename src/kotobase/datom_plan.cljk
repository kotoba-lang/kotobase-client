(ns kotobase.datom-plan
  "How an `[s p o]` pattern becomes a Datomic-API index read. Pure, portable,
  and deliberately **not** a child namespace of `kotobase.datom-source`.
  ...
  `kotobase.datom-source` is `.cljs`: it prefetches through `kotobase.client/datoms`,
  which is a Promise-returning HTTP client. This one is `.cljc` because the
  same decision is needed **in-process** — `kotobase-server`'s `do-datoms`
  calls `kotobase-peer`'s `hot-datoms` directly, has no HTTP client in the
  path, and runs its tests on the JVM. Requiring a `.cljs` namespace from
  there breaks the JVM build, and copying the plan into the server would put
  the one thing that must not drift — which index answers which bound
  positions — in two places.

  So: the plan lives here, the transports live where their transport does.
